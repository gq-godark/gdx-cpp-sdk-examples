// Minimal `.env` loader bundled with the GoDark C++ SDK as a development
// convenience for sample / smoke programs and quick-start CLIs.
//
...
// package), so `KEY=value ./my_app` still overrides a `.env` entry.
//
// Not used by the trading core itself — production code should use a real
// secret store. Lives under `godark::examples::` for backwards compatibility
// with the previous bundled-in-examples copy.
//
// Usage:
